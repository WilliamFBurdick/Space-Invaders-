...
{
	if (m_MovingRight) m_TC->getLocation().x += m_Speed * fps;
	else m_TC->getLocation().x -= m_Speed * fps;
	//update the collider
	m_RCC->setOrMoveCollider(m_TC->getLocation().x, m_TC->getLocation().y, m_TC->getSize().x, m_TC->getSize().y);
	m_TimeSinceLastShot += fps;

	//is the middoe of the invader above the player?
	...
	{
		//has the invader waited long enoug since the last shot
