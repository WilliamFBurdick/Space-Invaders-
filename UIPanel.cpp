...
UIPanel::UIPanel(Vector2i res, int x, int y, float width, float height, int alpha, int red, int green, int blue)
{
	...
	//set the center to be the middle of the width and height
...
void UIPanel::addButton(float x, float y, int width, int height, int red, int green, int blue, string label) { m_Buttons.push_back(make_shared<Button>(Vector2f(x, y), width, height, red, green, blue, label)); }
...
void UIPanel::draw(RenderWindow& window)
{
	...
	if (!m_Hidden) {
		...
		for (auto it = m_Buttons.begin(); it != m_Buttons.end(); ++it) (*it)->draw(window);
	}
}
