...
void BulletUpdateComponent::spawnForPlayer(Vector2f spawnPosition)
{
	...
	//tweak the y location based on the height of the bullet, the x location is already tweaked to thecenter of the player
...
void BulletUpdateComponent::spawnForInvader(Vector2f spawnPosition)
{
	...
	m_TC->getLocation().x = spawnPosition.x;
	//tweak the y location based on the height of the bullet, the x location is already the center of the invader
...
void BulletUpdateComponent::update(float fps)
{
	if (m_IsSpawned)
	{
		if (m_MovingUp) m_TC->getLocation().y -= m_Speed * fps;
		else m_TC->getLocation().y += m_Speed / m_AlienBulletSpeedModifier * fps;
		if (m_TC->getLocation().y > WorldState::WORLD_HEIGHT || m_TC->getLocation().y < -2) despawn();
		//update the collider
		m_RCC->setOrMoveCollider(m_TC->getLocation().x, m_TC->getLocation().y, m_TC->getSize().x, m_TC->getSize().y);
	}
}