...
void GameObjectFactoryPlayMode::buildGameObject(GameObjectBlueprint& bp, std::vector<GameObject>& gameObjects)
{
	...
	for (it; it != end; ++it)
	{
		if (*it == "Transform") gameObject.addComponent(make_shared<TransformComponent>(bp.getWidth(), bp.getHeight(), Vector2f(bp.getLocationX(), bp.getLocationY())));
		else if (*it == "Player Update") gameObject.addComponent(make_shared<PlayerUpdateComponent>());
		else if (*it == "Invader Update") gameObject.addComponent(make_shared<InvaderUpdateComponent>());
		else if (*it == "Bullet Update") gameObject.addComponent(make_shared<BulletUpdateComponent>());
