...
void GameInputHandler::handleGamepad()
{
	...
	if (Joystick::isButtonPressed(0, 1)) mBButtonPressed = true;
...
void GameInputHandler::handleKeyPressed(Event& event, RenderWindow& window)
{
	...
	if (event.key.code == Keyboard::Left) m_PUC->moveLeft();
	if (event.key.code == Keyboard::Right) m_PUC->moveRight();
	if (event.key.code == Keyboard::Up) m_PUC->moveUp();
	if (event.key.code == Keyboard::Down) m_PUC->moveDown();
}

void GameInputHandler::handleKeyReleased(Event& event, RenderWindow& window)
{
	if (event.key.code == Keyboard::Left) m_PUC->stopLeft();
	if (event.key.code == Keyboard::Right) m_PUC->stopRight();
	if (event.key.code == Keyboard::Up) m_PUC->stopUp();
	if (event.key.code == Keyboard::Down) m_PUC->stopDown();
