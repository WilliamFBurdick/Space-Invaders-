...
void GameScreen::initialize()
{
	...
	//store all the bullet locations and inialize all spawners in invaders
	int i = 0;
	auto it = m_ScreenManagerRemoteControl->getGameObjects().begin();
	auto end = m_ScreenManagerRemoteControl->getGameObjects().end();
	for (it; it != end; ++it)
	{
		if ((*it).getTag() == "bullet") m_BulletObjectLocations.push_back(i);
...
void GameScreen::update(float fps)
{
	...
	if (!m_GameOver)
	{
		if (m_WaitingToSpawnBulletForPlayer)
		{
			...
			m_NextBullet++;
			if (m_NextBullet == m_BulletObjectLocations.size()) m_NextBullet = 0;
		}
...
void GameScreen::draw(RenderWindow& window)
{
	...
	auto it = m_ScreenManagerRemoteControl->getGameObjects().begin();
	auto end = m_ScreenManagerRemoteControl->getGameObjects().end();
	for (it; it != end; ++it) (*it).draw(window);

	Screen::draw(window);
}

BulletSpawner* GameScreen::getBulletSpawner() { return this; }